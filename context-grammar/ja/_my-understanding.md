...
## 1. 中核アーキテクチャ — 6-Stage Decision Pipeline
...
Stage 5: Negotiation Gate
         ...
         コア公式: Final Autonomy = min(User Autonomy Setting, Gate Autonomy Ceiling)
         意味すり合わせUIプリミティブ: Interpretation Preview / Assumption Cards / Priority Toggle
         ← Autonomy Resolution(旧Stage 6)とNegotiation Design(旧Always-On Layer)を吸収

Stage 6: AX Patterns × Agent Action Lifecycle
         23パターン、各パターンにLifecycle Verbタグ付き
         Lifecycle Verbs: Inform / Recommend / Plan / Prepare / Act / Monitor / Adapt
         実際のUI出力、通知、外部APIコール、サイドエフェクトも担当
         ← Agentic Response(旧Stage 8)を吸収
```

**2つのAlways-On Layer / 常に働くレイヤー(全Stageを横断 — ナンバリングなし):**
...
> **Negotiation Design について:** 意味すり合わせの責務は概念的には常在する。だが機構はGateに局在するため、独立の第3層では描かず、Gateが体現する形で表現する（否定ではなく再配置）。
...
**Autonomy Resolution(Stage 5 Gate出力):** `Final Autonomy = min(User Autonomy Setting, Gate Autonomy Ceiling)` ← Stage 6(旧)は廃止。この公式はGate(Stage 5)の出力として統合された。

---

## 1.7 Agent Action Lifecycle(Stage 6のサブ分類)
...
## 4. Brain (Always-On Layer / 常に働くレイヤー) — 3層メモリ
...
## 6. Stage 6: AX × Lifecycle — 23 AX Patterns
...
## 7. Always-On Layer: Trust Design
